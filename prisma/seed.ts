...
// Use DIRECT_URL (session-mode, port 5432) — Prisma upsert uses prepared statements
// which are incompatible with pgBouncer transaction mode (DATABASE_URL, port 6543).
...
async function main() {
  ...
  console.log(`✅  ${books.length} books upserted`);
  ...
  console.log(`✅  ${pastPapers.length} past papers upserted`);

  // ── Stationery (15) — all with required type field ─────────────────────────
  const stationery = [
    {
