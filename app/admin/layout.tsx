...
// ── Active-link helper (server) ───────────────────────────────────────────────
// We use a simple server component that reads the pathname via headers.
// For client-side active state without "use client" on the layout,
// we pass the href as a data attribute and use CSS :has() or accept the simpler approach below.
...
function AdminNavLink({
  ...
  // Active styling is applied client-side via the Link's aria-current in a client wrapper.
  // For this server component we use a static link — the teal active highlight is applied
  // via CSS in globals.css targeting [data-active="true"].
  return (
    <Link
      href={href}
